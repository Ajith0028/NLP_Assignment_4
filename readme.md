...
Compute raw attention scores using to prevent large values from making the softmax unstable.

Apply softmax to convert the scores into normalized attention weights.

Multiply the attention weights by 
𝑉
V to obtain the final output vectors.
